                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $data['no']++ }}</td>
                            <td>
                                <strong>{!! Str::limit($item['title'][App::getLocale()], 60) !!}</strong>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="box-btn flex-wrap justify-content-end">
                                    <button type="button" class="btn icon-btn btn-sm btn-success restore" onclick="$(this).find('#form-restore').submit();" 
                                        ...
                                        <form action="{{ route('content.post.restore', ['sectionId' => $item['section_id'], 'id' => $item['id']])}}" method="POST" id="form-restore-{{ $item['id'] }}">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                    </button>
    ...
    $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/content/'+sectionId+'/post/'+id+'/permanent?is_trash=yes',
                        method: 'DELETE',
                        headers: {
