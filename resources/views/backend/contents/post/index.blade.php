        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="box-btn">
                    ...
                    <a href="{{ route('content.category.index', ['sectionId' => $data['section']['id']]) }}" class="btn btn-success w-icon" title="@lang('module/content.category.manage')">
                        <i class="fi fi-rr-ballot"></i> <span>@lang('module/content.category.manage')</span>
                    </a>
                    @endif
                    @can ('content_post_create')
                    <a href="{{ route('content.post.create', array_merge(['sectionId' => $data['section']['id']], $queryParam)) }}" class="btn btn-main w-icon" title="@lang('global.add_attr_new', [
                        'attribute' => __('module/content.post.caption')
                        ])">
                        <i class="fi fi-rr-add"></i>
                        <span>@lang('module/content.post.caption')</span>
                    </a>
                    @endcan
                    ...
                    <a href="{{ route('content.post.trash', ['sectionId' => $data['section']['id']]) }}" class="btn btn-dark w-icon" title="@lang('global.trash')">
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            @endif
                            <th>@lang('module/content.post.label.title')</th>
                            @if (Auth::user()->hasRole('developer|super') || Auth::user()->can('content_categories') && config('cms.module.content.category.active') == true && $data['section']['config']['show_category'] == true)
                        ...
                        <tr id="{{ $item['id'] }}" style="cursor: move;">
                            ...
                            <td>
                                <div class="box-btn flex-wrap justify-content-center">
                                    ...
                                    <a href="{{ route('media.index', ['moduleId' => $item['id'], 'moduleType' => 'content_post']) }}" class="btn icon-btn btn-sm btn-main" title="@lang('master/media.caption')">
                                        <i class="fi fi-rr-add-folder"></i>
                                    </a>
                                    @endif
                                    @can('content_post_update')
                                    <a href="{{ route('content.post.edit', array_merge(['sectionId' => $item['section_id'], 'id' => $item['id']], $queryParam)) }}" class="btn icon-btn btn-sm btn-success" title="@lang('global.edit_attr', [
                                        'attribute' => __('module/content.post.caption')
                                    ])">
            ...
            <div class="card-footer justify-content-center justify-content-lg-between align-items-center flex-wrap">
                <div class="text-muted mb-3 m-lg-0">
                    ...
                    <strong>{{ $data['posts']->firstItem() }}</strong> - 
                    <strong>{{ $data['posts']->lastItem() }}</strong> 
                    @lang('pagination.of')
