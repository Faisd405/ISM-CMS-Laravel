                ...
                <table class="table card-table table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th>@lang('module/content.category.label.field1')</th>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-success icon-btn btn-sm restore" onclick="$(this).find('#form-restore').submit();" title="@lang('global.restore')" data-id="{{ $item['id'] }}">
                                    ...
                                    <form action="{{ route('content.category.restore', ['sectionId' => $item['section_id'], 'id' => $item['id']])}}" method="POST" id="form-restore-{{ $item['id'] }}">
                                        @csrf
                                        @method('PUT')
                                    </form>
                                </button>
                                <button type="button" class="btn btn-danger icon-btn btn-sm swal-delete" data-section-id="{{ $item['section_id'] }}" data-id="{{ $item['id'] }}" title="@lang('global.delete')">
    ...
    $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/content/'+sectionId+'/category/'+id+'/permanent?is_trash=yes',
                        method: 'DELETE',
                        headers: {
