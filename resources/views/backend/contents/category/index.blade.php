    ...
    <div class="col-xl-12 col-lg-12 col-md-12">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="my-2">
                    @lang('module/content.category.text')
                </h5>
                <div class="box-btn">
                    ...
                    <a href="{{ route('content.post.index', ['sectionId' => $data['section']['id']]) }}" class="btn btn-success w-icon" title="@lang('module/content.post.manage')">
                        <i class="fi fi-rr-typewriter"></i> <span>@lang('module/content.post.manage')</span>
                    </a>
                    @endif
                    @can ('content_category_create')
                    <a href="{{ route('content.category.create', array_merge(['sectionId' => $data['section']['id']], $queryParam)) }}" class="btn btn-main w-icon" title="@lang('global.add_attr_new', [
                        'attribute' => __('module/content.category.caption')
                        ])">
                        <i class="fi fi-rr-add"></i>
                        <span>@lang('module/content.category.caption')</span>
                    </a>
                    @endcan
                    ...
                    <a href="{{ route('content.category.trash', ['sectionId' => $data['section']['id']]) }}" class="btn btn-dark w-icon" title="@lang('global.trash')">
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th>@lang('module/content.category.label.name')</th>
                        ...
                        <tr id="{{ $item['id'] }}" style="cursor: move;">
                            ...
                            <td class="text-center">
                                ...
                                <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="badge badge-{{ $item['publish'] == 1 ? 'main' : 'warning' }}"
                                    ...
                                    <form action="{{ route('content.category.publish', ['sectionId' => $item['section_id'], 'id' => $item['id']]) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                    </form>
                                </a>
                            ...
                            <td>
                                <div class="box-btn flex-wrap justify-content-center">
                                    ...
                                    <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="btn icon-btn btn-sm btn-dark" title="@lang('global.position')">
                                        ...
                                        <form action="{{ route('content.category.position', ['sectionId' => $item['section_id'], 'id' => $item['id'], 'position' => ($item['position'] - 1)]) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                    </a>
                            ...
                            <td>
                                <div class="box-btn flex-wrap justify-content-end">
                                    @can('content_category_update')
                                    <a href="{{ route('content.category.edit', array_merge(['sectionId' => $item['section_id'], 'id' => $item['id']], $queryParam)) }}" class="btn icon-btn btn-sm btn-success" title="@lang('global.edit_attr', [
                                        'attribute' => __('module/content.category.caption')
                                    ])">
    ...
    $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/content/'+sectionId+'/category/'+id+'/soft',
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        icon: 'success',
                        text: "@lang('global.alert.delete_success', ['attribute' => __('module/content.category.caption')])"
                    }).then(() => {
                        window.location.reload();
                    })
                } else {
