    ...
    <div class="col-xl-12 col-lg-12 col-md-12">
        ...
        <div class="card">
            <div class="card-body d-flex flex-wrap justify-content-between">
                ...
                <div class="d-flex w-100 w-xl-auto">
                    ...
                    @endcan
                    @role('super')
                    <a href="{{ route('link.category.trash') }}" class="btn btn-secondary icon-btn-only-sm btn-sm" title="@lang('global.trash')">
        ...
        <div class="card">
            <div class="card-header with-elements">
                <h5 class="card-header-title mt-1 mb-0">@lang('module/link.category.text')</h5>
            </div>
                ...
                <table class="table card-table table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th>@lang('module/link.category.label.field1')</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="{{ route('link.category.read', ['slugCategory' => $item['slug']]) }}" title="@lang('global.view_detail')" target="_blank">
                            ...
                            <td class="text-center">
                                ...
                                <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="badge badge-{{ $item['publish'] == 1 ? 'primary' : 'warning' }}"
                                    ...
                                    <form action="{{ route('link.category.publish', ['id' => $item['id']]) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                    </form>
                                </a>
                            ...
                            <td class="text-center">
                                @if (Auth::user()->can('link_category_update') && $item->min('position') != $item['position'])
                                <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="btn icon-btn btn-sm btn-dark" title="@lang('global.position')">
                                    ...
                                    <form action="{{ route('link.category.position', ['id' => $item['id'], 'position' => ($item['position'] - 1)]) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                    </form>
                                </a>
                                ...
                                @endif
                                @if (Auth::user()->can('link_category_update') && $item->max('position') != $item['position'])
                                <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="btn icon-btn btn-sm btn-dark" title="@lang('global.position')">
                            ...
                            <td class="text-center">
                                @can('link_medias')
                                <a href="{{ route('link.media.index', ['categoryId' => $item['id']]) }}" class="btn icon-btn btn-sm btn-success" title="@lang('module/link.media.caption')">
                                ...
                                @endcan
                                @if (Auth::user()->hasRole('super|support|admin') && config('cms.module.link.category.approval') == true)
                                <a href="javascript:void(0);" onclick="$(this).find('#form-approval').submit();" class="btn icon-btn btn-sm btn-{{ $item['approved'] == 1 ? 'danger' : 'primary' }}" title="{{ $item['approved'] == 1 ? __('global.label.flags.0') : __('global.label.flags.1')}}">
                                    ...
                                    <form action="{{ route('link.category.approved', ['id' => $item['id']]) }}" method="POST" id="form-approval">
                                        @csrf
                                        @method('PUT')
                                    </form>
                                </a>
    ...
    $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/link/category/'+id+'/soft',
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        type: 'success',
                        text: "@lang('global.alert.delete_success', ['attribute' => __('module/link.category.caption')])"
                    }).then(() => {
