                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $data['no']++ }}</td>
                            <td>
                                {!! Str::limit($item['label'][App::getLocale()], 30) !!}
                            </td>
                            <td>
                                <code>{{ $item['name'] }}</code>
                            </td>
                            ...
                            <td>
                                <div class="box-btn flex-wrap justify-content-end">
                                    <button type="button" class="btn icon-btn btn-sm btn-success restore" onclick="$(this).find('#form-restore').submit();" 
                                        ...
                                        <form action="{{ route('inquiry.field.restore', ['inquiryId' => $item['inquiry_id'], 'id' => $item['id']])}}" method="POST" id="form-restore-{{ $item['id'] }}">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                    </button>
 ...
 $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/inquiry/' + inquiryId + '/field/'+ id +'/permanent?is_trash=yes',
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        icon: 'success',
                        text: "{{ __('global.alert.delete_success', ['attribute' => __('module/inquiry.field.caption')]) }}"
                    }).then(() => {
