                            ...
                            <td>
                                <div class="box-btn flex-wrap justify-content-end">
                                    <button type="button" class="btn btn-success icon-btn btn-sm restore" onclick="$(this).find('#form-restore').submit();" 
                                        ...
                                        <form action="{{ route('district.restore', ['provinceCode' => $item['province_code'], 'cityCode' => $item['city_code'], 'id' => $item['id']])}}" method="POST" id="form-restore-{{ $item['id'] }}">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                    </button>
                                    <button type="button" class="btn btn-danger icon-btn btn-sm swal-delete" 
                                        ...
                                        data-city-code="{{ $item['city_code'] }}" data-id="{{ $item['id'] }}"
                                        data-toggle="tooltip" data-placement="bottom"
    ...
    $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/regional/province/' + provinceCode + '/city/' + cityCode + '/district/' + id + '/permanent?is_trash=yes',
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        icon: 'success',
                        text: "{{ __('global.alert.delete_success', ['attribute' => __('module/regional.district.caption')]) }}"
                    }).then(() => {
