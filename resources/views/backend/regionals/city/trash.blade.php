                ...
                <table class="table card-table table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" style="width: 50px;">@lang('module/regional.city.label.field1')</th>
                            <th>@lang('module/regional.city.label.field2')</th>
                            <th>@lang('module/regional.city.label.field3')</th>
                            <th>@lang('module/regional.city.label.field4')</th>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-success icon-btn btn-sm restore" onclick="$(this).find('#form-restore').submit();" title="@lang('global.restore')" data-id="{{ $item['id'] }}">
                                    ...
                                    <form action="{{ route('city.restore', ['provinceCode' => $item['province_code'], 'id' => $item['id']])}}" method="POST" id="form-restore-{{ $item['id'] }}">
                                        @csrf
                                        @method('PUT')
                                    </form>
                                </button>
                                <button type="button" class="btn btn-danger icon-btn btn-sm swal-delete" data-province-code="{{ $item['province_code'] }}" data-id="{{ $item['id'] }}" title="@lang('global.delete')">
                ...
                <div class="card-footer">
                    <div class="row align-items-center">
                        <div class="col-lg-6 m--valign-middle">
                            @lang('pagination.showing') : <strong>{{ $data['cities']->firstItem() }}</strong> - <strong>{{ $data['cities']->lastItem() }}</strong> @lang('pagination.of')
    ...
    $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/regional/province/' + provinceCode + '/city/' + id + '/permanent?is_trash=yes',
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        type: 'success',
                        text: "{{ __('global.alert.delete_success', ['attribute' => __('module/regional.city.caption')]) }}"
                    }).then(() => {
