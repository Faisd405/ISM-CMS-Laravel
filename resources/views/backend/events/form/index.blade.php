            ...
            <div class="card-header">
                ...
                <div class="modal modal-export fade" id="modals-export">
                    <div class="modal-dialog">
                        <form class="modal-content" action="{{ route('event.form.export', ['eventId' => $data['event']['id']]) }}" method="POST">
                            @csrf
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th>@lang('module/event.form.label.ip_address')</th>
                            <th>@lang('module/event.label.register_code')</th>
                            @foreach ($data['fields']->take(3) as $item)
                                <th>{{ $item->fieldLang('label') }}</th>
                            @endforeach
                            <th class="text-center" style="width: 80px;">@lang('global.status')</th>
                            <th class="text-center" style="width: 80px;">@lang('module/event.form.label.exported')</th>
                            <th style="width: 230px;">@lang('module/event.form.label.submit_time')</th>
                            ...
                            <td>
                                <div class="box-btn flex-wrap justify-content-end">
                                    ...
                                    <button type="button" class="btn btn-danger icon-btn btn-sm swal-delete" title="@lang('global.delete_attr', [
                                        ...
                                        data-event-id="{{ $item['event_id'] }}" data-id="{{ $item['id'] }}">
...
<script>
    $(document).ready(function () {
        ...
        $('.read-form').on('click', function () {
            ...
            if (status == 0) {
                $.ajax({
                    url: '/admin/event/' + eventId + '/form/' + id + '/status',
                    type: 'PUT',
                });
            }
        });
        ...
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/event/'+eventId+'/form/'+id,
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        icon: 'success',
                        text: "@lang('global.alert.delete_success', ['attribute' => __('module/event.form.caption')])"
                    }).then(() => {
