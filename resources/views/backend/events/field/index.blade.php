    ...
    <div class="col-xl-12 col-lg-12 col-md-12">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="my-2">
                    @lang('module/event.field.text')
                </h5>
                <div class="box-btn">
                    @can('event_field_create')
                    <a href="{{ route('event.field.create', array_merge(['eventId' => $data['event']['id']], $queryParam)) }}" class="btn btn-main w-icon" title="@lang('global.add_attr_new', [
                        'attribute' => __('module/event.field.caption')
                        ])">
                        <i class="fi fi-rr-add"></i> <span>@lang('module/event.field.caption')</span>
                    </a>
                    @endcan
                    ...
                    <a href="{{ route('event.field.trash', ['eventId' => $data['event']['id']]) }}" class="btn btn-dark w-icon" title="@lang('global.trash')">
            ...
            <div class="card-header">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item">
                        <span>{{ Str::upper(__('module/event.field.caption')) }}</span>
                    </li>
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th>@lang('module/event.field.label.label')</th>
                            <th style="width: 210px;">@lang('module/event.field.label.name')</th>
                        ...
                        <tr id="{{ $item['id'] }}" style="cursor: move;">
                            ...
                            <td>
                                <div class="box-btn flex-wrap justify-content-end">
                                    @can('event_field_update')
                                    <a href="{{ route('event.field.edit', array_merge(['eventId' => $item['event_id'], 'id' => $item['id']], $queryParam)) }}" class="btn icon-btn btn-sm btn-success" title="@lang('global.edit_attr', [
                                        'attribute' => __('module/event.field.caption')
                                    ])">
                                    ...
                                    @can('event_field_delete')
                                        ...
                                        @endif
                                    @endcan
                                    @if (Auth::user()->hasRole('developer|super') && config('cms.module.gallery.file.approval') == true)
            ...
            <div class="card-footer justify-content-center justify-content-lg-between align-items-center flex-wrap">
                <div class="text-muted mb-3 m-lg-0">
                    ...
                    <strong>{{ $data['fields']->firstItem() }}</strong> - 
                    <strong>{{ $data['fields']->lastItem() }}</strong> 
                    @lang('pagination.of')
    ...
    $(function () {
        ...
        $(".drag").sortable({
            ...
            update : function (event, ui) {
                ...
                $.ajax({
                    data: {'datas' : data},
                    url: '/admin/event/'+eventId+'/field/sort',
                    type: 'POST',
                    dataType:'json',
                    success: function(){
                        refreshNeeded = true;
