                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-success icon-btn btn-sm restore" onclick="$(this).find('#form-restore').submit();" title="@lang('global.restore')" data-id="{{ $item['id'] }}">
                                    ...
                                    <form action="{{ route('gallery.album.restore', ['id' => $item['id']])}}" method="POST" id="form-restore-{{ $item['id'] }}">
                                        @csrf
                                        @method('PUT')
                                    </form>
                                </button>
    ...
    $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/gallery/album/' + id + '/permanent?is_trash=yes',
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        type: 'success',
                        text: "{{ __('global.alert.delete_success', ['attribute' => __('module/gallery.album.caption')]) }}"
                    }).then(() => {
