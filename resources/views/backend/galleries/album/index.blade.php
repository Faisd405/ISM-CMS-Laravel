    ...
    <div class="col-xl-12 col-lg-12 col-md-12">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="my-2">
                    @lang('module/gallery.album.text')
                </h5>
                <div class="box-btn">
                    @can ('gallery_categories')
                        @if (Auth::user()->hasRole('developer|super') || config('cms.module.gallery.category.active') == true)
                        <a href="{{ route('gallery.category.index') }}" class="btn btn-success w-icon" title="@lang('module/gallery.category.manage')">
                            <i class="fi fi-rr-ballot"></i> <span>@lang('module/gallery.category.manage')</span>
                        </a>
                        @endif
                    @endcan
                    @can ('gallery_album_create')
                    <a href="{{ route('gallery.album.create', $queryParam) }}" class="btn btn-main w-icon" title="@lang('global.add_attr_new', [
                        'attribute' => __('module/gallery.category.caption')
                        ])">
                        <i class="fi fi-rr-add"></i>
                        <span>@lang('module/gallery.album.caption')</span>
                    </a>
                    @endcan
                    ...
                    <a href="{{ route('gallery.album.trash') }}" class="btn btn-dark w-icon" title="@lang('global.trash')">
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th>@lang('module/gallery.album.label.name')</th>
                            @if (Auth::user()->hasRole('developer|super') || config('cms.module.gallery.category.active') == true)
                        ...
                        <tr id="{{ $item['id'] }}" style="cursor: move;">
                            ...
                            <td>
                                ...
                                <div class="box-btn flex-wrap justify-content-center">
                                    ...
                                    @endif
                                    @if (Auth::user()->can('gallery_album_update') && $item->max('position') != $item['position'])
                                    <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="btn icon-btn btn-sm btn-dark" title="@lang('global.position')">
                            ...
                            <td>
                                <div class="box-btn flex-wrap justify-content-end">
                                    @can('gallery_files')
                                    <a href="{{ route('gallery.file.index', ['albumId' => $item['id']]) }}" class="btn icon-btn btn-sm btn-main" title="@lang('module/gallery.file.caption')">
                                        <i class="fi fi-rr-folder"></i>
                                    </a>
                                    @endcan
                                    @can('gallery_album_update')
                                    <a href="{{ route('gallery.album.edit', array_merge(['id' => $item['id']], $queryParam)) }}" class="btn icon-btn btn-sm btn-success" title="@lang('global.edit_attr', [
            ...
            <div class="card-footer justify-content-center justify-content-lg-between align-items-center flex-wrap">
                <div class="text-muted mb-3 m-lg-0">
                    ...
                    <strong>{{ $data['albums']->firstItem() }}</strong> - 
                    <strong>{{ $data['albums']->lastItem() }}</strong> 
                    @lang('pagination.of')
    ...
    $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/gallery/album/'+id+'/soft',
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        icon: 'success',
                        text: "@lang('global.alert.delete_success', ['attribute' => __('module/gallery.album.caption')])"
                    }).then(() => {
                        window.location.reload();
                    })
                } else {
