    ...
    <div class="col-xl-12 col-lg-12 col-md-12">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="my-2">
                    @lang('module/gallery.category.text')
                </h5>
                <div class="box-btn">
                    @can ('gallery_category_create')
                    <a href="{{ route('gallery.category.create', $queryParam) }}" class="btn btn-main w-icon" title="@lang('global.add_attr_new', [
                        'attribute' => __('module/gallery.category.caption')
                        ])">
                        <i class="fi fi-rr-add"></i>
                        <span>@lang('module/gallery.category.caption')</span>
                    </a>
                    @endcan
                    ...
                    <a href="{{ route('gallery.category.trash') }}" class="btn btn-dark w-icon" title="@lang('global.trash')">
                ...
                <table class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th>@lang('module/gallery.category.label.name')</th>
                        ...
                        <tr id="{{ $item['id'] }}" style="cursor: move;">
                            ...
                            <td class="text-center">
                                ...
                                <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="badge badge-{{ $item['publish'] == 1 ? 'main' : 'warning' }}"
                                    ...
                                    <form action="{{ route('gallery.category.publish', ['id' => $item['id']]) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                    </form>
                                </a>
                            ...
                            <td>
                                ...
                                <div class="box-btn flex-wrap justify-content-center">
                                    ...
                                    <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="btn icon-btn btn-sm btn-dark" title="@lang('global.position')">
                                        ...
                                        <form action="{{ route('gallery.category.position', ['id' => $item['id'], 'position' => ($item['position'] - 1)]) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                    </a>
                                    ...
                                    @endif
                                    @if (Auth::user()->can('gallery_category_update') && $item->max('position') != $item['position'])
                                    <a href="javascript:void(0);" onclick="$(this).find('form').submit();" class="btn icon-btn btn-sm btn-dark" title="@lang('global.position')">
                            ...
                            <td class="text-center">
                                <div class="box-btn flex-wrap justify-content-end">
                                    @can('gallery_category_update')
                                    <a href="{{ route('gallery.category.edit', array_merge(['id' => $item['id']], $queryParam)) }}" class="btn icon-btn btn-sm btn-success" title="@lang('global.edit_attr', [
                                        'attribute' => __('module/gallery.category.caption')
                                    ])">
    ...
    $(document).ready(function () {
        $('.swal-delete').on('click', function () {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return $.ajax({
                        url: '/admin/gallery/category/'+id+'/soft',
                        method: 'DELETE',
                        headers: {
                ...
                if (response.value.success) {
                    Swal.fire({
                        icon: 'success',
                        text: "@lang('global.alert.delete_success', ['attribute' => __('module/gallery.category.caption')])"
                    }).then(() => {
                        window.location.reload();
                    })
                } else {
