...
class WidgetRepository
{
    ...
    public function getModuleData($data)
    {
        ...
        if ($data['type'] == 'page') {
            ...
            $filter['approved'] = 1;
            $module = [
                'page' => App::make(PageRepository::class)->getPage(['id' => $data['moduleable_id']]),
                'childs' => App::make(PageRepository::class)->getPageList($filter, true, $data['content']['child_limit'], false, [], $orderBy)
            ];
        }

        if ($data['type'] == 'content_section') {
            $module['section'] = App::make(ContentRepository::class)->getSection(['id' => $data['moduleable_id']]);

            $filter['publish'] = 1;
            $filter['approved'] = 1;
            $filter['section_id'] = $data['moduleable_id'];
            $module['categories'] = App::make(ContentRepository::class)->getCategoryList($filter, true, $data['content']['category_limit'], false, [], $orderBy);
            ...
            $module['posts'] = App::make(ContentRepository::class)->getPostList($filter, true, $data['content']['post_limit'], false, [], $orderBy);
        }

        if ($data['type'] == 'content_category') {
            $module['category'] = App::make(ContentRepository::class)->getCategory(['id' => $data['moduleable_id']]);
        ...
        if ($data['type'] == 'banner') {
            $module['banner'] = App::make(BannerRepository::class)->getBanner(['id' => $data['moduleable_id']]);

            $filter['publish'] = 1;
            $filter['approved'] = 1;
            $filter['banner_id'] = $data['moduleable_id'];

            $module['files'] = App::make(BannerRepository::class)->getFileList($filter, true, $data['content']['banner_limit'], false, [], $orderBy);
        }

        if ($data['type'] == 'gallery_category') {
            $module['category'] = App::make(GalleryRepository::class)->getCategory(['id' => $data['moduleable_id']]);

            $filter['publish'] = 1;
            $filter['approved'] = 1;
            $filter['gallery_category_id'] = $data['moduleable_id'];

            $module['albums'] = App::make(GalleryRepository::class)->getAlbumList($filter, true, $data['content']['album_limit'], false, [], $orderBy);
        }

        if ($data['type'] == 'gallery_album') {
            $module['album'] = App::make(GalleryRepository::class)->getAlbum(['id' => $data['moduleable_id']]);

            $filter['publish'] = 1;
            $filter['approved'] = 1;
            $filter['gallery_album_id'] = $data['moduleable_id'];

            $module['files'] = App::make(GalleryRepository::class)->getFileList($filter, true, $data['content']['file_limit'], false, [], $orderBy);
        }

        if ($data['type'] == 'document') {
            $module['document'] = App::make(DocumentRepository::class)->getDocument(['id' => $data['moduleable_id']]);

            $filter['publish'] = 1;
            $filter['approved'] = 1;
            $filter['document_id'] = $data['moduleable_id'];

            $module['files'] = App::make(DocumentRepository::class)->getFileList($filter, true, $data['content']['file_limit'], false, [], $orderBy);
        }

        if ($data['type'] == 'link') {
            $module['link'] = App::make(LinkRepository::class)->getLink(['id' => $data['moduleable_id']]);

            $filter['publish'] = 1;
            $filter['approved'] = 1;
            $filter['link_id'] = $data['moduleable_id'];

            $module['medias'] = App::make(LinkRepository::class)->getMediaList($filter, true, $data['content']['media_limit'], false, [], $orderBy);
        }

        if ($data['type'] == 'inquiry') {
            $module['inquiry'] = App::make(InquiryRepository::class)->getInquiry(['id' => $data['moduleable_id']]);

            $filter['publish'] = 1;
            $filter['approved'] = 1;
            $filter['inquiry_id'] = $data['moduleable_id'];

            $module['fields'] = App::make(InquiryRepository::class)->getFieldList($filter, false, 0, false, [], $orderBy);
        }

        if ($data['type'] == 'event') {
            $module['event'] = App::make(EventRepository::class)->getEvent(['id' => $data['moduleable_id']]);

            $filter['publish'] = 1;
            $filter['approved'] = 1;
            $filter['event_id'] = $data['moduleable_id'];

            $module['fields'] = App::make(EventRepository::class)->getFieldList($filter, false, 0, false, [], $orderBy);
        }
